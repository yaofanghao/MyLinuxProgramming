 ...
 * @LastEditTime: 2023-07-24 21:26:26
...
* 栈段
  * 包括局部变量和函数调用的上下文等
* 堆和文件映射段的内存动态分配
  * 比如C标准库的 malloc() 或 mmap() ，就可以分别在堆和文件映射段动态分配内存。
...
| 类型 | 来源 | 特点 |
|------|------|------|
| 外部中断（硬件中断） | 硬件设备（键盘、网卡、磁盘） | 异步、可屏蔽（IF 标志位） |
| 内部中断（异常） | CPU 内部 | 同步 |
| 缺页异常（Page Fault） | 访问未映射的虚拟地址 | 可恢复，OS 加载页面后重新执行指令 |
| 除零异常 | 算术错误 | 不可恢复，进程收到 SIGFPE |
| 系统调用（int 0x80 / syscall） | 用户程序主动触发 | 同步，切换至内核态 |

### 中断处理流程

1. CPU 保存当前执行上下文（CS、RIP、RSP 等寄存器压栈）
2. CPU 查 IDT（中断描述符表）找到对应处理函数入口
3. 关闭中断（硬件自动）或保留中断标志
4. 执行中断处理函数（上半部 → 下半部）
5. 恢复上下文，iretq/sysretq 返回用户态

### 上半部 vs 下半部

* **上半部（Top Half）**：快速处理，关中断，做最小必要操作（ACK 设备、拷贝数据）
* **下半部（Bottom Half）**：慢速处理，开中断，做主要工作（协议处理、唤醒进程）
* 下半部实现方式：softirq（软中断）、tasklet、workqueue（工作队列）

## Linux CFS 调度器

### 核心数据结构

* 每个 CPU 有一个运行队列（cfs_rq）
* 就绪进程以 `vruntime` 为键值存储在红黑树中
* 每次调度选红黑树最左节点（vruntime 最小）
...
* nice 0 权重为 1024，nice -20 权重为 88761（约 87 倍）

### 调度延迟

* 目标延迟（targeted latency）：默认 6ms（可配），保证每个进程都能被调度一次
* 最小粒度（min granularity）：默认 0.75ms，防止频繁上下文切换

## RCU（Read-Copy-Update）

* 适用场景：读多写少的共享数据结构（如路由表、文件系统缓存）
* **原理**：
  * 读操作无锁（直接访问指针）
  * 写操作：创建副本 → 修改副本 → 发布新指针 → 等待所有读端完成 → 回收旧版本
  * 读端通过 `rcu_read_lock()` / `rcu_read_unlock()` 标记临界区
* **宽限期（Grace Period）**：所有 CPU 都经历过一次上下文切换，确保旧版本无人引用
* **优点**：读性能极高，无锁、无内存屏障（某些架构）

## 中断亲和性（IRQ Affinity）

* 将特定中断绑定到特定 CPU，提高缓存命中率
* `/proc/irq/[irq_num]/smp_affinity` 写入 CPU 掩码
* `/proc/irq/[irq_num]/smp_affinity_list` 写入 CPU 列表
* 网卡多队列 + RPS（Receive Packet Steering）实现多核分摊网络中断

## NUMA 架构

* **UMA（统一内存访问）**：所有 CPU 共享内存总线，访问延迟一致
* **NUMA（非统一内存访问）**：每个 CPU 有自己的本地内存，访问本地内存快，访问远程内存慢
* **查看 NUMA 拓扑**：`numactl --hardware`、`lstopo`
* **优化**：绑核（taskset）、内存绑定（numactl --membind）、大页（HugePages）

## eBPF（Extended Berkeley Packet Filter）

* 在内核中安全运行沙箱化程序，无需修改内核代码
* **应用场景**：
  * 性能追踪（bcc / bpftrace）
  * 网络包过滤（XDP 代替 iptables）
  * 安全监控（seccomp、LSM BPF）
  * 容器网络（Cilium）
* **常用工具**：`bpftrace`、`bcc`（`execsnoop`、`biotop`、`tcplife`）

## 操作系统常见面试题

| 问题 | 答案要点 |
|------|---------|
| 进程和线程的区别？ | 进程是资源分配单位，线程是调度单位；进程有独立地址空间，线程共享地址空间 |
| 用户态和内核态切换的开销？ | 保存/恢复寄存器、TLB 刷新、栈切换、权限检查，约数百纳秒到数微秒 |
| 为什么需要虚拟内存？ | 隔离进程地址空间、简化内存管理、允许运行大于物理内存的程序 |
| 大页（HugePages）的好处？ | 减少页表项数量（2MB/1GB 页），降低 TLB miss，提升性能 |
